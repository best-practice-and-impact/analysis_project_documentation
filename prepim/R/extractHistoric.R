...
#' @title Extract historic accounts from reference file
#'
#' @description Extracts net GFCF data from a pre-defined CSV file.
#'
...
#' @import tidyr
#' @import readr
#' @return tidy data.frame of net balanced accounts with \code{Sector},
#'  \code{Industry}, and \code{Asset} variables, plus the time series.
#' @export
extractHistoric <- function(path) {
  ...
  categoryHeadings <- c("Sector", "Industry", "Asset")
  ...
  # Add a Prices column (all "CP")
  x <- dplyr::mutate(x, Prices = "CP")

  # Standarise Industry codings from e.g. "1" to "01"
  x <- dplyr::mutate(x, Industry = if_else(nchar(Industry) == 1, paste0("0", Industry), Industry))
